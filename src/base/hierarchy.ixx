...
export class Hierarchy {
  public:
	...
	GameDataSource source = GameDataSource::none;
	std::vector<mpq::MPQ> mpq_archives;
	...
	bool open_casc(const fs::path& directory) {
		...
		if (open) {
			source = GameDataSource::casc;
			aliases.load(open_file("filealiases.json").value());
		}
		return open;
	}

	bool open_mpq(const fs::path& directory) {
		warcraft_directory = directory;
		...
		auto add_mpq = [&](const fs::path& path) {
			if (!fs::exists(path)) {
				return;
			}
			mpq::MPQ archive;
		...
		add_mpq(warcraft_directory / "war3xlocal.mpq");
		add_mpq(warcraft_directory / "War3x.mpq");
		add_mpq(warcraft_directory / "war3x.mpq");
		add_mpq(warcraft_directory / "War3Local.mpq");
		add_mpq(warcraft_directory / "war3local.mpq");
		add_mpq(warcraft_directory / "War3.mpq");
		add_mpq(warcraft_directory / "war3.mpq");

		// Tileset MPQs (A.mpq, B.mpq, ...)
		for (char c = 'A'; c <= 'Z'; c++) {
			add_mpq(warcraft_directory / (std::string(1, c) + ".mpq"));
			add_mpq(warcraft_directory / (std::string(1, static_cast<char>(std::tolower(c))) + ".mpq"));
		}

		if (mpq_archives.empty()) {
			return false;
		}

		source = GameDataSource::mpq;
		return true;
	}
	...
	bool is_classic() const {
		return source == GameDataSource::mpq;
	}
	...
	bool file_exists(const fs::path& path) const {
		...
		if (fs::exists("data/overrides" / path) || (local_files && fs::exists(root_directory / path))) {
			return true;
		}
		...
		if (source == GameDataSource::mpq) {
			...
			return aliases.exists(path.string()) ? file_exists(aliases.alias(path.string())) : false;
		}
